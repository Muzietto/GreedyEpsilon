 ...
 * @author mfaustinelli
 */
public class AlgoMultiEpsilonInjectableStretcher {

    private final Writer _writer;

    public AlgoMultiEpsilonInjectableStretcher(Writer writer) {
        _writer = writer;
    }

    public void campaignAlgorithm(List<EpsilonGreedy> algos, List<BernoulliArm> arms, Integer numSims, Integer horizon) throws IOException {

        Iterator<EpsilonGreedy> itt = algos.iterator();
        while (itt.hasNext()) {
            EpsilonGreedy algo = itt.next();
            System.out.println("epsilon is " + algo.epsilon());
            testAlgorithm(algo, arms, numSims, horizon);
        }
    ...
    public void testAlgorithm(EpsilonGreedy algo, List<BernoulliArm> arms, Integer numSims, Integer horizon) throws IOException {
        String currEpsilon = algo.epsilon().toString();
        TableRow<Integer> chosenArm = new TableRow<Integer>("chosenArm_"+currEpsilon, horizon);
        TableRow<Integer> bestArm = new TableRow<Integer>("bestArm_"+currEpsilon, horizon);
        RunningAverageTableRow bestArmPercentage = new RunningAverageTableRow("goodArmPercentage_"+currEpsilon, horizon);
        TableRow<Double> reward = new TableRow<Double>("reward_"+currEpsilon, horizon);
        RunningAverageTableRow averageReward = new RunningAverageTableRow("averageReward_"+currEpsilon, horizon);
        TableRow<Double> cumReward = new TableRow<Double>("cumReward_"+currEpsilon, horizon);
        TableRow<Integer> simNum = new TableRow<Integer>("simNum_"+currEpsilon, horizon);
        TableRow<Integer> draw = new TableRow<Integer>("draw_"+currEpsilon, horizon);
